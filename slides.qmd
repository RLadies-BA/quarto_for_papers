...
If you already have a peper of your own (with different r files with code) go to your RStudio Terminal and create a new project from scratch
...
The comments hypotesis true alllows to activate for my html file, so it allows to add the comments as we have seen before and for the pdf we also have differnt options in order to kepp the reproductibility: we can keep the the markdown files genreates (keep-md), the notbook files genreated (keep-ipynd), or as I'm choosimg here the intermediate text files that are genrested.
...
::: callout-tip
Quarto support differnt syntax variotions to include page numbers, chapters, exclude author names and other cool posibilities.
You can learn more about it the Citation documentation at https://quarto.org/docs/authoring/citations.html
:::
...
::: notes
Quarto can generate cross references that keep track of numbering and linking for you.
The general syntax for a cross reference is an `@` followed by a `label`.
For example, to reference the timeline figure, you could write: To render the cross references you have to include a word to indicate the type of object being referenced, like “Figure” or “Table”, so this:
:::
...
## Authoring embeded code from other notebooks

::: notes
We can do all the calculations in another notebook (it can be r document but it coould be also be a python notebook if you already are wrking with python) and we can embeded that into outr paper, by refereing to it the codechunck options and embeded notebook options
:::
...
![](img/manuscript-emebeded-notebook-figures.gif)
...
{{< fa brands linkedin >}} [www.linkedin.com/in/betsabe1984](https://www.linkedin.com/in/betsabe1984)

{{< fa envelope title="An envelope" >}} [betsabecohen\@gmail.com](mailito:betsabecohen@gmail.com)

<br>

{{< fa brands linkedin >}} [www.linkedin.com/in/betsabe1984](https://www.linkedin.com/in/jesica-formoso/)

{{< fa envelope title="An envelope" >}} [betsabecohen\@gmail.com](mailito:jesica.formoso@gmail.com)
